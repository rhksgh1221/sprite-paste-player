...
// Order: pixelate -> color grade -> cel shade -> recolor -> alpha generation/key/cleanup/smoothing -> outline.
...
export const FILTER_PRESETS=[
  ...
  ['Key Out Black',{enabled:true,colorKey:true,keyColor:'#000000',keyTolerance:12,keySoftness:18,alphaSmooth:true,alphaSmoothRadius:2}],
...
function colorKeyFactor(f,r,g,b){const k=hexToRgb(f.keyColor||'#000000'),maxD=Math.sqrt(3)*255,tol=clampPct(f.keyTolerance)/100*maxD,soft=clampPct(f.keySoftness)/100*maxD,dr=r-k[0],dg=g-k[1],db=b-k[2],dist=Math.sqrt(dr*dr+dg*dg+db*db);if(soft<=0)return dist<=tol?0:1;return clamp01((dist-tol)/soft)}
...
export function applyFilters(src,dst,filters,frame=0){
  ...
  const needGrade=!!(f.brightness||f.contrast||f.saturation||f.hue),celOn=!!f.cel,rampOn=!!f.ramp,aOn=!!f.alphaCut,alphaGen=!!f.alphaGen,keyOn=!!f.colorKey,smoothOn=!!f.alphaSmooth,alphaScale=(f.alphaValue==null?100:f.alphaValue)/100;
  if(needGrade||celOn||rampOn||aOn||alphaGen||keyOn||smoothOn||alphaScale!==1){
    ...
    for(let i=0;i<d.length;i+=4){let a=d[i+3],or=d[i],og=d[i+1],ob=d[i+2],r=or,g=og,b=ob;
      if(a===0&&!alphaGen&&!keyOn)continue;
      ...
      if(keyOn)a=clamp8(a*colorKeyFactor(f,or,og,ob));
